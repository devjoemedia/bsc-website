...
const PartnersA = () => {
  const { partners_section } = {
    partners_section: {
      ...
      subtitle: "At Because She Can, we believe that collaboration is the key to creating meaningful and lasting impact. Our strategic partners are organizations that share our vision of bridging the digital gender divide in Africa. Together, we work to provide our community with the best resources, tools, and opportunities to succeed ",
      images: [
      ...
      <div className=' lg:col-start-4 lg:col-end-8 space-y-5'>
        <Marquee autoFill={true} direction='left' gradientWidth={100} gradient={true} speed={40} className='flex gap-5'>
          {partners_section?.images?.map((item: string, idx: number) => (
            <Image alt="partners" width={1200} height={100} src={`${item}`} key={idx} className='space-y-2 h-22 w-[90px] ml-3' />
          ))}
        </Marquee>
