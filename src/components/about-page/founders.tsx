...
const founders = [
  ...
  },
  {
    ...
    image: "https://ui-avatars.com/api/?name=Kweyakie+Afi+Blebo&background=723b69&color=fff&size=256"
  }
];
  ...
  return (
    <section className="py-24 bg-bsc-yellow/20 relative">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-primary font-semibold tracking-wider uppercase text-sm mb-2 block">Leadership</span>
          <h2 className="text-4xl md:text-5xl font-recoleta font-bold text-gray-900">Hear from Our Founders</h2>
        </motion.div>
          ...
          {founders.map((founder, index) => (
            ...
            >
              <div className="flex items-center gap-6 mb-8">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden bg-primary shrink-0 relative">
                    <img src={founder.image} alt={founder.name} className="w-full h-full object-cover" />
                </div>
                <div>
                  <h3 className="text-2xl font-recoleta font-bold text-gray-900 mb-1">{founder.name}</h3>
                  <p className="text-primary font-medium text-sm md:text-base mb-1">{founder.role}</p>
                  <p className="text-gray-500 text-sm flex items-center gap-2">
                    {founder.title}
                    <a href={founder.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#0a66c2] transition-colors">
                      <Linkedin size={18} />
                    </a>
                  </p>
                </div>
              ...
              <div className="relative flex-grow">
                <span className="absolute top-0 left-0 -translate-x-4 -translate-y-4 text-6xl text-primary/10 font-recoleta">&ldquo;</span>
