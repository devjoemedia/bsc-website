...
export default function VolunteerLayout({ data = defaultData }: { data?: VolunteerData }) {
  return (
    <div className="bg-white">
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden bg-linear-to-br from-primary via-primary-light to-primary">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center pt-24 pb-16">
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-lg text-white/90 leading-relaxed max-w-2xl mx-auto">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-4xl font-recoleta font-bold text-gray-900 mb-6">{data.mission.title}</h2>
              <p className="text-gray-600 leading-relaxed text-lg">{data.mission.content}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-gray-50 rounded-3xl p-10 border border-gray-100 shadow-sm relative overflow-hidden">
              <div className="absolute -right-10 -bottom-10 w-40 h-40 bg-primary/5 rounded-full blur-3xl"></div>
              <h2 className="text-3xl font-recoleta font-bold text-primary mb-6 relative z-10">{data.empowerment.title}</h2>
              <p className="text-gray-700 leading-relaxed relative z-10">{data.empowerment.content}</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-gray-50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
           <div className="text-center mb-16">
             <h2 className="text-4xl md:text-5xl font-recoleta font-bold text-gray-900 mb-4">Open Opportunities</h2>
             <p className="text-gray-600 max-w-2xl mx-auto text-lg">Find the perfect role where your skills meet our community's needs.</p>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             {data.roles.map((role, idx) => (
               ...
               >
                 <h3 className="text-2xl font-recoleta font-bold text-gray-900 mb-4 group-hover:text-primary transition-colors">{role.title}</h3>
                 <p className="text-gray-600 mb-8 flex-grow">{role.description}</p>
                 
                 <div className="bg-gray-50 rounded-2xl p-6 mb-8 mt-auto">
                   <h4 className="text-sm font-bold tracking-wider text-gray-900 uppercase mb-4">Requirements</h4>
                 ...
                 <button className="w-full py-4 rounded-xl bg-primary text-white font-bold hover:bg-primary-light transition-colors flex items-center justify-center gap-2 group/btn cursor-pointer">
                   Apply for this role
